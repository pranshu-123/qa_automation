...
public class TezAppsDetailsPage {
    ...
    /**
     * Method to click the first app in jobs table , navigate to the details page.
     * and verify Type value .
     */
    public String verifyType(TezAppsDetailsPageObject tezApps) {
        String typeValue = tezApps.getType.getText();
        logger.info("Tez Type is " + typeValue);
        waitExecuter.sleep(5000);
        waitExecuter.waitUntilPageFullyLoaded();
        Assert.assertNotSame("", typeValue, "Tez Type is not displayed in the Table");
        return typeValue;
    }
    ...
    public String verifyStatus(TezAppsDetailsPageObject tezApps) {
        String typeValue = tezApps.getStatus.getText();
        logger.info("Tez Status is " + typeValue);
        waitExecuter.sleep(5000);
        waitExecuter.waitUntilPageFullyLoaded();
        Assert.assertNotSame("", typeValue, "Tez Status is not displayed in the Table");
        return typeValue;
    }
    ...
    public String verifyAppname(TezAppsDetailsPageObject tezApps) {
        String Appname = tezApps.getAppname.getText();
        logger.info("Tez Status is " + Appname);
        waitExecuter.sleep(5000);
        waitExecuter.waitUntilPageFullyLoaded();
    ...
    public String verifyAppid(TezAppsDetailsPageObject tezApps) {
        String Appid = tezApps.getAppid.getText();
