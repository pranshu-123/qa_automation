...
public class ChargeBackImpala {
	...
	public boolean isDataSorted(ImpalaJobTableColumn impalaJobTableColumn, Boolean isReversed) {
		...
		if (clickOnLastOfPaginationIfExists(isReversed)) {
			try {
...

		{
			...
			return actualDates.containsAll(sortedList);
			...
			return actualTime.containsAll(sortedList);
		} else {
			...
			return actualDataString.containsAll(sortedList);
		}
	}
	 ...
	 */
	public Boolean clickOnLastOfPaginationIfExists(boolean flag) {
		Boolean isPaginationExist = false;
		JavaScriptExecuter.scrollOnElement(driver, applicationsPageObject.isPaginationPresent);
		JavaScriptExecuter.scrollOnElement(driver, applicationsPageObject.isPaginationPresent);
		if (applicationsPageObject.isPaginationPresent.isEnabled() && flag==true) {
