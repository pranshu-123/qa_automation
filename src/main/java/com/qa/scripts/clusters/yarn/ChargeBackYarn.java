...
public class ChargeBackYarn {
	 ...
	 */
	public void closeGroupByOptionsExcept(String groupByOption) {
		waitExecuter.sleep(2000);
		clickOnGroupBySearchBox();
		List<WebElement> elements = chargebackImpalaPageObject.closeGroupByOptionButton;
		for(int i=0; i<elements.size(); i++) {
			if (!elements.get(i).findElement(By.xpath("//parent::li")).getText().contains(groupByOption)) {
	...
	public void selectOptionsInGroupBy(String optionName){
		...
		for (int i = 0; i < listOfWebElemnts.size(); i++) {
			...
			if(listOfWebElemnts.get(i).getText().equalsIgnoreCase(optionName)){
