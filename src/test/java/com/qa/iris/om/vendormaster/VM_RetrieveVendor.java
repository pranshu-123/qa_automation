...
public class VM_RetrieveVendor extends MainAccelerator{

	private static final Logger LOGGER = Logger.getLogger(VM_RetrieveVendor.class.getName());


	@Test(groups = Categories.PURCHASE_ORDER)
	public void TC_VM_12_retrieveVendor() {
		test = extent.createTest("TC_VM_12_retrieveVendor", "Create New Vendor");
		test.assignCategory("VendorMaster");
		Log.startTestCase("TC_VM_12_retrieveVendor");
		...
		vendorMasterWorkflow.createVendor("activevendormaster");
		vendorMasterWorkflow.createNewVendorMaster();
		vendorMasterWorkflow.searchVendor("activevendormaster");
		vendorMasterWorkflow.activateVendor();
		LOGGER.info("Vendor activated !!!");
		vendorMasterWorkflow.validateActivation();
		vendorMasterWorkflow.retrieveVendor();
		test.log(Status.PASS, "Vendor retrieved successfully");
	}
