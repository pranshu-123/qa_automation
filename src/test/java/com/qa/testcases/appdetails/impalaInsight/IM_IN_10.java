...
@Marker.All
public class IM_IN_10 extends BaseClass {

    private final LoggingUtils loggingUtils = new LoggingUtils(IM_IN_10.class);

    /**
     * Verify Impala insights SQL Non Pruned Partition Events
     */

    @Test
    public void verifyNonPrunedPartitionEvents() {
        test = extent.startTest("IM_IN_10.verifyNonPrunedPartitionEvents",
                "Verify Impala application table having non pruned partition events. ");
        test.assignCategory("App Details - Impala");
        loggingUtils.info("Started test case: IM_IN_10.verifyNonPrunedPartitionEvents", test);
        ...
        try {
            ...
            if (appCount > 0) {
                ...
                appDetailsPage.selectEventFilter(ImpalaEventTypes.SqlNonPrunedPartitionsEvent);
                ...
                loggingUtils.info("Expected Title on page - " + PageConstants.EventTypes.SqlNonPrunedPartitionsEvent, test);
                appDetailsPage.close();
                Assert.assertTrue(titles.contains(PageConstants.EventTypes.SqlNonPrunedPartitionsEvent),
                        "Does not contain title as expected in analysis tab- "+PageConstants.EventTypes.SqlNonPrunedPartitionsEvent);
                test.log(LogStatus.PASS, "SqlNonPrunedPartitionsEvent verified ");
            } else {
