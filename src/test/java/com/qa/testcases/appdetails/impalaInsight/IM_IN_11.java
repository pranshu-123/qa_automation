...
public class IM_IN_11 extends BaseClass {

    private final LoggingUtils loggingUtils = new LoggingUtils(IM_IN_11.class);

    /**
     * Verify Impala insights Time Skew Events
     */

    @Test
    public void verifyTimeSkewEvents() {
        test = extent.startTest("IM_IN_11.verifyTimeSkewEvents",
                "Verify Impala application table having time skew events. ");
        test.assignCategory("App Details - Impala");
        loggingUtils.info("Started test case: IM_IN_11.verifyTimeSkewEvents", test);
        ...
        try {
            ...
            if (appCount > 0) {
                ...
                appDetailsPage.selectEventFilter(ImpalaEventTypes.ImpalaTimeSkewEvent);
                ...
                loggingUtils.info("Expected Titles on page - " + PageConstants.EventTypes.ImpalaTimeSkewEvent, test);
                appDetailsPage.close();
                Assert.assertTrue(titles.contains(PageConstants.EventTypes.ImpalaTimeSkewEvent),
                        "Does not contain title as expected in analysis tab- " + PageConstants.EventTypes.ImpalaTimeSkewEvent);
                test.log(LogStatus.PASS, "ImpalaTimeSkewEvent verified ");
            } else {
