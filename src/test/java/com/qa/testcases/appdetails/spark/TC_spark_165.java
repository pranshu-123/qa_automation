...
public class TC_spark_165 extends BaseClass {
	/**
	 * 1. Execute a spark application 2. From Unravel UI open the spark application.
	 * 3. Wait for the spark application to complete. 4. Click on actions and verify
	 * if the user can execute the action "Load Diagnostics".
	 */
	...
	public void TC_spark_165_verifyLoadDiagnosticAction(String clusterId) {
		...
		if (appCount > 0) {
			...
			wait.waitUntilElementClickable(sparkAppsDetailsPageObject.resetButton);
			sparkAppsDetailsPageObject.resetButton.click();
			wait.waitUntilElementClickable(sparkAppsDetailsPageObject.resetButton);
		} else {
