...
@Marker.All
public class TC_QU_15 extends BaseClass {
    private static final Logger LOGGER = Logger.getLogger(TC_QU_15.class.getName());

    @Test(description = "P0-Validate Vcores graph is loaded for selected queue")
    public void validateVcoresQueueGraph() {
        test = extent.startTest("TC_QU_15.validateVcoresQueueGraph", "Validate Vcore graphs for a queue");
        ...
        try {
            ...
            if (qaPageObject.getQueueList.size() > 0) {
                ...
                List<String> expectedVcoresToolTip = queueAnalysis.expectedVcoresToolTips();
                test.log(LogStatus.INFO, "Expected tool tip values- " + expectedVcoresToolTip);
                // Get actual list of tool tips
                test.log(LogStatus.INFO, "Get actual list of tool tips");
                LOGGER.info("Get actual list of tool tips");
                List<String> actualToolTipValues = queueAnalysis.actualVcoresToolTips();
                test.log(LogStatus.INFO, "Actual tool tip values- " + actualToolTipValues);

                Assert.assertTrue(actualToolTipValues.containsAll(expectedVcoresToolTip),
                        "Tool tip of vcores graph did not match set of values");
