...
@Marker.All
public class TC_QU_17 extends BaseClass {
    private static final Logger LOGGER = Logger.getLogger(TC_QU_17.class.getName());

    @Test(description = "P0-Validate Vcores graph data points for selected queue")
    public void validateVcoresDataPoint() {
        test = extent.startTest("TC_QU_17.validateVcoresDataPoint", "Validate Vcore graphs data points for a queue");
        ...
        try {
            ...
            if (qaPageObject.getQueueList.size() > 0) {
                ...
                queueAnalysis.clickOnDatapointOfGraph(driver, qaPageObject.vcoresGraph);
