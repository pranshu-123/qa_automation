...
public class TC_CTX_28 extends BaseClass {
    ...
    public void TC_CTX_28_verifyQueueFilter() {
        ...
        topX.clearFilter();
        waitExecuter.sleep(3000);
        String queueFilter = topX.getFilterDropDowns().get(0).getText();
        ...
        LOGGER.info("Report is started successfully for queue: " + queueFilter, test);
        ...
        topX.validateLatestReport("Queue", queueFilter);
        test.log(LogStatus.PASS, "Verified filter is displayed for queue on TopX report.");
    }
}
