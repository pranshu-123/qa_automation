...
public class TC_CD_05 extends BaseClass {
    private static final Logger LOGGER = Logger.getLogger(TC_CD_05.class.getName());

    @Test
    public void validateByQueueCount() {
        test = extent.startTest("TC_CD_05.validateByQueueCount",
                "Verify the Cluster Discovery Queue count from pie chart");
        ...
        try {
            ...
            Assert.assertNotNull(totalQueues, "Total Queues count is null");
            Assert.assertTrue(totalJobs >= 0, "The total count expected Queues is null");
            test.log(LogStatus.PASS, "Queue counts and Job counts are greater than 0");

        } catch (NoSuchElementException ex) {
            Assert.assertTrue(cdPageObject.noDataByQueue.getText().trim().equals("No Data Available."),
