...
public class TC_FR_02 extends BaseClass {
  ...
  public void validateFileReportNavigation(String clusterId) {
    ...
    Assert.assertTrue(fileReportsPageObject.selectedLargeFileOpt.isDisplayed(),"Large File option not displayed");

    MouseActions.clickOnElement(driver, fileReportsPageObject.tinyFile);
    waitExecuter.waitUntilPageFullyLoaded();
    Assert.assertTrue(fileReportsPageObject.selecteTinyFileOpt.isDisplayed()," Tiny file option not displayed");

    MouseActions.clickOnElement(driver, fileReportsPageObject.mediumFile);
    waitExecuter.waitUntilPageFullyLoaded();
    Assert.assertTrue(fileReportsPageObject.selecteMediumFileOpt.isDisplayed()," Medium file option not displayed");

    MouseActions.clickOnElement(driver, fileReportsPageObject.emptyFile);
    waitExecuter.waitUntilPageFullyLoaded();
    Assert.assertTrue(fileReportsPageObject.selecteEmptyFileOpt.isDisplayed()," Empty file option not displayed");
  }
}
