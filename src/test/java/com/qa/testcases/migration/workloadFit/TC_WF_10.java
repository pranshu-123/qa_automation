...
@Marker.WorkloadFit
@Marker.All
public class TC_WF_10 extends BaseClass {
    private static final Logger LOGGER = Logger.getLogger(TC_WF_10.class.getName());

    @Test
    public void validateJobTypeAndUser() {
        test = extent.startTest("TC_WF_10.validateJobTypeAndUsers",
                "Verify that Unravel UI should generate a Workload Fit report and Job Type and Users pie-chart should be displayed.");
        ...
        for (WebElement appUserName : appNamesInUserTypes) {
            userList.add(appUserName.getText());
        }
        LOGGER.info("Name of user present are- " + userList.toString());
        test.log(LogStatus.INFO, "Name of user present are- " + userList.toString());
        Assert.assertNotNull(userList, "There are no apps in User Type pie chart.");
