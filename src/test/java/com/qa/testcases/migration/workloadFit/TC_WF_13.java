...
@Marker.WorkloadFit
@Marker.All
public class TC_WF_13 extends BaseClass {
    private static final Logger LOGGER = Logger.getLogger(TC_WF_13.class.getName());

    @Test
    public void validateUserTypeErrorMessage() {
        test = extent.startTest("TC_WF_13.validateUserTypeErrorMessage",
                "Verify that Unravel UI should populate a warning message to select at least one User Type.");
        ...
        fit.deselectAllCheckboxType(fitPageObject.AllTagOfUserTypes, fitPageObject.selectAllUserTypes);

        // Assert that error message is displayed on de-selecting user Type
        Assert.assertTrue(fitPageObject.noJobSelectedErrorMessage.size() > 0,
                "On de-selecting all text box the warning message to select at least one user is not displayed");
        test.log(LogStatus.PASS,
                "On de-selecting all user Type, warning message to select at-least one user was displayed");
    }
}