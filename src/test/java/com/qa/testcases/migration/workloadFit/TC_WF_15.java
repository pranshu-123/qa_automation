...
@Marker.WorkloadFit
@Marker.All
public class TC_WF_15 extends BaseClass {
    private static final Logger LOGGER = Logger.getLogger(TC_WF_15.class.getName());

    @Test
    public void validateTagsTypeErrorMessage() {
        test = extent.startTest("TC_WF_15.validateTagsTypeErrorMessage",
                "Verify that Unravel UI should populate a warning message to select at least one Tags Type.");
        ...
        fit.selectLast7Days();
        // De-Select All Tags types run in last 7 days
        test.log(LogStatus.INFO, "De-Select All Tag types run in last 7 days.");
        LOGGER.info("De-Select All Tag types run in last 7 days.");
        fit.deselectAllCheckboxType(fitPageObject.AllTagOfTagsTypes, fitPageObject.selectAllTagTypes);

        // Assert that error message is displayed on de-selecting Tags Type
        Assert.assertTrue(fitPageObject.noJobSelectedErrorMessage.size() > 0,
                "On de-selecting all check box the warning message to select at least one Tag is not displayed");
        test.log(LogStatus.PASS,
                "On de-selecting all Tags Type, warning message to select at-least one Tag was displayed");
    }
}